...
// Run with: npx vitest run test/evals/ragas/02-vapi-answer-relevancy.test.ts --reporter=verbose

// VAPI Chat API response shape
interface VapiChatResponse {
  id: string;
  assistantId: string;
  messages: Array<{ role: string; content: string }>;
  // The assistant's reply is in `output`, not `messages`
...
// Note: VAPI does not expose the retrieved document chunks from its internal
// knowledge base in the API response, so ragas/faithfulness (which requires
// `contexts`) cannot be used here. We use ragas/answer_relevancy instead,
// which only needs `input` and `output`.
...
describe('LangWatch - Ragas VAPI Answer Relevancy Evaluations', () => {
  it('should evaluate VAPI agent answer relevancy for medical helpline queries', async () => {
    const vapiPrivateKey = process.env.VAPI_PRIVATE_KEY;
    if (!vapiPrivateKey) throw new Error('VAPI_PRIVATE_KEY is not set');
    ...
    const experiment = await langwatch.experiments.init(
      'macular-society-vapi-ragas-answer-relevancy',
    );
    ...
    await experiment.run(DATASET, async ({ item, index }) => {
      ...
      if (!response.ok) {
        throw new Error(
          `VAPI API returned ${response.status}: ${await response.text()}`,
        );
      }

      const data: VapiChatResponse = await response.json();
      const answer = data.output[0]?.content ?? '';
