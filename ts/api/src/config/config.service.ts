...
export class ConfigService {
  ...
  private loadConfig(): EnvConfig {
    ...
    const embedding: EmbeddingConfig = {
      provider: getBool('OPENAI_ENABLE', false) ? 'openai' : 'ollama',
    ...
    const ollama: OllamaConfig = {
      url: getRequired('OLLAMA_URL'),
      model: getRequired('OLLAMA_MODEL'),
      timeout: getInt('OLLAMA_TIMEOUT', 600),
    };
